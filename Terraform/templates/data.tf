...
data "google_projects" "environment_projects" {
  filter = "parent.id:${split("/", var.folder_id)[1]} name:*${var.project_suffix}* labels.application_name=${var.business_code}-${var.env_code}-application labels.environment=${var.environment} lifecycleState=ACTIVE"
}
